...
//批量处理参数
//字符格式： [{工单号, 工作项id, 用户号}， {工单号, 工作项id, 用户号}...]；
String batchparams = request.getParameter("batchparams");
...
<js:load scriptPath="workflows/ticket/js/batchhandler/workSheetListUi.js"/>
<js:load scriptPath="workflows/ticket/js/batchhandler/workSheetListEv.js"/>


<script type="text/javascript">
 FunctionMgr.load({mainfn : com.zoomlion.hjsrm.workflows.TickeBatchHandler ,
 	cfg:{pId : '<%=pId %>' , processId : '<%=processId %>' , workItemId : '<%=workItemId %>' , activityName : '<%=activityName %>' ,
				...
				url : '<%=url%>' , userId : '<%=userId%>' ,  busitype : '<%=busitype%>', busipkid : '<%=busipkid%>',
				applyinfopkid : '<%=applyinfopkid%>' , worklistinfopkid : '<%=worklistinfopkid%>', batchparams : Ext.decode('<%=batchparams%>')
	}
 });
</script>
</head>
