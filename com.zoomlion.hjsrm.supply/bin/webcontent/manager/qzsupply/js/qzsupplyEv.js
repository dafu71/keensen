/*******************************************************************************
 * 版权所有： 中联重科环境产业公司 描 述： 潜在供应商界面 创建日期： 2014-12-12 作 者： 刘鑫
 ******************************************************************************/
com.zoomlion.hjsrm.qzsupply.QzsupplyMgr.prototype.destroy = function() {
	this.viewWindow.destroy();
	this.editWindow.destroy();
	this.qzsupplyaddPanel.destroy();
	this.supplyQuerySetPanel.destroy();
	this.qzsupplyQuerySetPanel.destroy();
	this.qzstoreTel.destroy();
	this.storeTel.destroy();
	this.inputqzsupplyWindow.destroy();
	this.supplyQueryTel.destroy();
	this.storeMulti.destroy();
	this.storeqzMulti.destroy();

}
com.zoomlion.hjsrm.qzsupply.QzsupplyMgr.prototype.initEvent = function() {
	// 增加查询事件
	...
	// this.storeqzMulti.on('load', function() {
	// Ext.getCmp(mySupplyMaterialid).selectAll(); // 全选
	// Ext.getCmp(qzSupplyMaterialid).setValue(stra); // 默认选中王五
	// });
...
myChart.setOption(option);
	...
	com.zoomlion.hjsrm.qzsupply.QzsupplyMgr.prototype.onSaveOk = function() {
		var _this = this;
		ValObj = {
		...
		var form = this.editWindow.items.items[0].form;
		...
		Ext.Ajax.request({
			method : "post",
			scope : this,
			url : 'com.zoomlion.hjsrm.supply.manager.supply.QzSupplyMgr.saveQzSupply.biz.ext',
			jsonData : {
				linshi : vals,
				qzdata : ValObj,
				list : supplys
	...
	com.zoomlion.hjsrm.qzsupply.QzsupplyMgr.prototype.onaddOk = function() {
		var _this = this;
		if (!this.qzsupplyaddPanel.form.isValid()) {
			return;
		}
		ValaObj = {
			lifnr : Ext.getCmp(this.lifnr1).getValue(),
			lprop : Ext.getCmp(this.lprop1).getValue(),
			ltype : Ext.getCmp(this.ltype1).getValue(),
			zzyyw : Ext.getCmp(this.zzyyw1).getValue(),
			lsort : Ext.getCmp(qzSupplyMaterialid).getValue()
		}

		var store = _this.listPanel.store;
		var qzrecords = this.qzstoreTel.getRange();
		var form = _this.qzsupplyaddPanel.form;
		var vals = _this.qzsupplyaddPanel.getForm().getValues();
		var qzsupplys = [];
		Ext.each(qzrecords, function(r) {
					qzsupplys.push(r.data);
				});
		...
		Ext.Ajax.request({
			...
			success : function(response, action) {
				...
				var result = Ext.decode(response.responseText);
				if (result.success) {
					_this.inputqzsupplyWindow.hide();
					Ext.Msg.alert("系统提示", "保存成功", function() {
						store.reload();
							// _this.listPanel.getForm().reset();
						});
 ...
 * 绑定修改按钮方法
 */
com.zoomlion.hjsrm.qzsupply.QzsupplyMgr.prototype.onEdit = function() {
	this.listPanel.onEdit();
};
com.zoomlion.hjsrm.qzsupply.QzsupplyMgr.prototype.onDel = function() {
	this.listPanel.onDel();
};
com.zoomlion.hjsrm.qzsupply.QzsupplyMgr.prototype.onAdd = function() {
	var _this = this;
	this.qzstoreTel.load();
	this.storeqzMulti.load();
	Ext.Ajax.request({
		method : "post",
		scope : this,
		url : 'com.zoomlion.hjsrm.supply.manager.supply.QzSupplyMgr.getQzsupplyid.biz.ext',
		success : function(resp) {
			var ret = Ext.decode(resp.responseText);
			if (ret.success) {
				var lifnr = ret.lifnr;
				this.inputqzsupplyWindow.items.items[0].form.findField('lifnr')
						.setValue(lifnr);
				var attachmentParams = {
					relationId : lifnr,
					groupId : 'swdjpaperwork'
				...
				Ext.getCmp(this.attachId1).setPostParams(attachmentParams1);
				Ext.getCmp(this.attachId2).setPostParams(attachmentParams2);
				Ext.getCmp(this.attachId3).setPostParams(attachmentParams3);
				Ext.getCmp(this.attachId4).setPostParams(attachmentParams4);
				Ext.getCmp(this.attachId5).setPostParams(attachmentParams5);
				Ext.getCmp(_this.attachId).loadParams = attachmentParams;
				Ext.getCmp(_this.attachId).loadAttachmentRemote();
				Ext.getCmp(_this.attachId1).loadParams = attachmentParams1;
				Ext.getCmp(_this.attachId1).loadAttachmentRemote();
				Ext.getCmp(_this.attachId2).loadParams = attachmentParams2;
				Ext.getCmp(_this.attachId2).loadAttachmentRemote();
				Ext.getCmp(_this.attachId3).loadParams = attachmentParams3;
				Ext.getCmp(_this.attachId3).loadAttachmentRemote();
				Ext.getCmp(_this.attachId4).loadParams = attachmentParams4;
				Ext.getCmp(_this.attachId4).loadAttachmentRemote();
				Ext.getCmp(_this.attachId5).loadParams = attachmentParams5;
				Ext.getCmp(_this.attachId5).loadAttachmentRemote();
			} else {
				Ext.Msg.alert('系统提示', '获取公共主键失败');
			}
		}
	}, this);
	this.inputqzsupplyWindow.show();
	this.qzstoreTel.load();
	this.storeqzMulti.load();

};

function qzsupplyShow(v) {
	var fid = Ext.getCmp("showqzSupplyInfo").fid;
	var fid1 = Ext.getCmp("showqzSupplyInfo").fid1;
	var fid2 = Ext.getCmp("showqzSupplyInfo").fid2;
	var fid3 = Ext.getCmp("showqzSupplyInfo").fid3;
	var fid4 = Ext.getCmp("showqzSupplyInfo").fid4;
	var fid5 = Ext.getCmp("showqzSupplyInfo").fid5;
	var record = new Ext.data.Record({
				"lifnr" : v
			});
	Ext.getCmp("showqzSupplyInfo").items.items[0].loadData(record);
	Ext.getCmp("showqzSupplyInfo").show();

	var ds = Ext.getCmp('qzsupplyTelPanel').getStore();
	ds.baseParams = {
		"entity/lifnr" : v
	};
	ds.load();
