 ...
 * 销毁组件
 */
com.zoomlion.hjsrm.tuyangbg.Purchase1Mgr.prototype.destroy = function() {
 ...
 * 初始化组件事件
 */
com.zoomlion.hjsrm.tuyangbg.Purchase1Mgr.prototype.initEvent = function() {

	var attachmentParams = {
		relationId : processinstid,
		groupId : 'tybgfile'
	};
	Ext.getCmp('DattachId').setPostParams(attachmentParams);
	Ext.getCmp('DattachId').loadParams = attachmentParams;
	Ext.getCmp('DattachId').loadAttachmentRemote();
	...
	this.listArchivesPanel.store.load();
 ...
 * 加载所有供应商
 */
com.zoomlion.hjsrm.tuyangbg.Purchase1Mgr.prototype.onLoadAll = function() {
	this.listEmpPanel.store.load();
}

/*
 * 选择人员信息
 */
com.zoomlion.hjsrm.tuyangbg.Purchase1Mgr.prototype.onSelect = function() {
	// this.listEmpPanel.fireEvent("select");
	var _this = this;
	var store = _this.listEmpPanel.store;
	var readerWindow = readerWindow
			|| new com.zoomlion.hjsrm.selectParticipantsWindow3({
				title : '选择供应商处理反馈人员',
				partTitle : '供应商处理反馈人员',
				divisionFront : "(",
				divisionBack : '),',
...
com.zoomlion.hjsrm.tuyangbg.Purchase1Mgr.prototype.onSelectArchives = function() {
	// this.listEmpPanel.fireEvent("select");
	var _this = this;
	var store = _this.listArchivesPanel.store;
	var readerWindow = readerWindow
			|| new com.zoomlion.hjsrm.selectParticipantsWindow3({
				title : '选择归档操作人员',
				partTitle : '归档操作人员',
				...
				divisionBack : '),',
				defaultValue : '',
				currentRolecode : CURRENTROLECODE2,
				listeners : {
					'callback' : function(v) {
						if (v) {
							var readers = Ext.util.Format.substr(v, 0, v.length
 ...
 * 移除操作员方法
 */
com.zoomlion.hjsrm.tuyangbg.Purchase1Mgr.prototype.onDelOperator = function() {
 ...
 * 移除归档操作员方法
 */
com.zoomlion.hjsrm.tuyangbg.Purchase1Mgr.prototype.onDelArchives = function() {
	if (!this.listArchivesPanel.getSelectionModel().getSelected()) {
		Ext.Msg.alert("系统提示", "没有选定数据，请选择数据行！");
	} else {
		Ext.Msg.confirm("系统提示", "您确定要移除操作员吗?", function(btnText) {
					if (btnText == "yes") {
						var rs1 = this.listArchivesPanel.getSelectionModel()
								.getSelections();
						this.listArchivesPanel.store.remove(rs1);
					}
				}, this);
	}
};
 ...
 * @return {Boolean}
 */
com.zoomlion.hjsrm.tuyangbg.Purchase1Mgr.prototype.onSave = function() {

	var _this = this;

	// 处理结果
	var nextItems1 = this.editPanel.nextItems.items.items[0].checked;
	var nextItems2 = this.editPanel.nextItems.items.items[1].checked;
	...
	if (nextItems2) {// 如果归档选中
		// 判断当前环节是否有多个工作项
		// var url =
		// 'com.zoomlion.hjsrm.techNotice.noticeMgr.queryWorkitems.biz.ext?workItemID='
		// + workitemid;
		// var result = Ext.ex.XMLHttpRequestEx.synchRequest("POST", url);
		// var ret = Ext.decode(result);
		// var listCount = ret.listCount;
		// if (listCount > 1) {// 如果当前环节有多个工作项，则不能直接归档
		// Ext.Msg.alert("系统提示", "当前环节有多个工作项，不能直接归档!");
		// return false;
		// } else {

		var records = this.listArchivesPanel.getStore().getRange();
		...
		if (records.length == 1) {
			...
			var selOperaotorids = this.listArchivesPanel.selModel
					.getSelections();
			if (selOperaotorids.length > 0) {
				Ext.each(selOperaotorids, function(record) {
							vals2[vals2.length] = {
 ...
 * 回退窗口关闭
 */
com.zoomlion.hjsrm.tuyangbg.Purchase1Mgr.prototype.onBackClose = function() {
 ...
 * 回退窗口，选择需要回退的环节
 */
com.zoomlion.hjsrm.tuyangbg.Purchase1Mgr.prototype.onBackItem = function() {
 ...
 * 回退窗口提交
 */
com.zoomlion.hjsrm.tuyangbg.Purchase1Mgr.prototype.onBack = function() {
	var _this = this;
 ...
 * 流程处理TAB页关闭
 */
com.zoomlion.hjsrm.tuyangbg.Purchase1Mgr.prototype.onClose = function() {
	closeMyTab();
}
