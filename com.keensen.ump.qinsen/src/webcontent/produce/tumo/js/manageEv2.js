com.keensen.ump.qinsen.produce.tumoMgr.prototype.initEvent = function() {
	...
	this.addDefectWindow.activeItem.mon(this.addDefectWindow.activeItem,
			'afterSave', function() {
				...
				this.addDefectWindow.tacheCause.setValue('涂膜');
				this.addDefectWindow.recorder.setValue(uname);
				if (ip == '172.16.10.10') {
					this.addDefectWindow.team.setValue(10020);
				}
				if (ip == '172.16.137.108') {
					this.addDefectWindow.team.setValue(30140);
				}
				if (ip == '172.16.137.192') {
					this.addDefectWindow.team.setValue(30220);
				}
				if (ip == '172.16.137.193') {
					this.addDefectWindow.team.setValue(30223);
				}
				if (ip == '172.16.136.182') {
					this.addDefectWindow.team.setValue(30240);
				}
...
com.keensen.ump.qinsen.produce.tumoMgr.prototype.onAddDefect = function() {
...
com.keensen.ump.qinsen.produce.tumoMgr.prototype.onChooseDefect = function() {
	this.chooseDefectWindow.show();
}

com.keensen.ump.qinsen.produce.tumoMgr.prototype.onChooseDefectOk = function() {
...
com.keensen.ump.qinsen.produce.tumoMgr.prototype.onAddColorb = function() {
	var A = this.listPanel;
	if (!A.getSelectionModel().getSelected()) {
		Ext.Msg.alert("系统提示", "没有选定数据，请选择数据行！")
	} else {
...
function defectView2(relationId, batchNo) {
	...
	var url = '/produce/quality/mpdefect/tmdefectlist.jsp?tmBatchNo=' + batchNo
