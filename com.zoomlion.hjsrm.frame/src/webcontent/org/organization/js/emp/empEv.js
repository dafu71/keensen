 ...
 * 事件与方法处理
 */
com.zoomlion.hjsrm.org.empMgr.prototype.initEvent = function() {
	/**
 ...
 *            e
 */
com.zoomlion.hjsrm.org.empMgr.prototype.onInput = function(btn, e) {
	...
	Ext.Ajax.request({
		...
		success : function(resp) {
			...
			code = res.data;
			if (param.orgid) {
				var cm = form.findField('emp/bizorgids');
				cm.loader.baseParams = {
					'query/dataorgid' : param['dataorgid'],
					'query/status' : 'running',
					'query/delflag' : 'n'
				};
				cm.setReadOnly(false);
 ...
 *            e
 */
com.zoomlion.hjsrm.org.empMgr.prototype.onDelete = function(btn, e) {
	// 保存this
	var me = this;
 ...
 * 恢复人员按钮点击处理
 */
com.zoomlion.hjsrm.org.empMgr.prototype.onRegainEmp = function() {
	// 保存this
	var me = this;
	var record = this.empGridPanel.selModel.getSelections();
	var data = [];
 ...
 *            e
 */
com.zoomlion.hjsrm.org.empMgr.prototype.onEdit = function() {
