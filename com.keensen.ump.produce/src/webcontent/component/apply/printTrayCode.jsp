...
<% 		}  %>
		...
		htmlArr.push('<tr>');
		htmlArr.push('<td colspan="2" style="height:16mm;vertical-align: middle; align:center">');
		htmlArr.push('<div id="canvas_' + <%=i %>
				+ '" style="margin:auto;width:50px;height:50px"></div>');
		htmlArr.push('</td>');
		
		htmlArr.push('</tr>');
		htmlArr.push('<tr>');
		htmlArr
				.push('<td colspan="2" style="height:16mm;text-align:center;padding:0px;margin:0px;align:center"">');
		
		htmlArr.push('<span id="tumoBatchStr" class="span_label">'
				+ '<%=list[i].get("trayCode") %>' + '</span>');
		htmlArr.push('</td>');
		htmlArr.push('</tr>');
