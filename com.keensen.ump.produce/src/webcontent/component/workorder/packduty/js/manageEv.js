com.keensen.ump.produce.component.workorder.PackdutyMgr.prototype.initEvent = function() {
	...
	this.packStore.on('load', function() {
				var records = _this.packStore.getRange();
				var arr = [];
				for (var i = 0; i < records.length; i++) {
					arr.push(records[i].data.code)
				}
				_this.defaultPackCode = arr.join(',')
			})
...
com.keensen.ump.produce.component.workorder.PackdutyMgr.prototype.onArrange = function() {
	this.listPanel4Arrange.store.removeAll();
	this.window4Arrange.show();
}

com.keensen.ump.produce.component.workorder.PackdutyMgr.prototype.onAddArrange = function() {
	this.chooseOrderWindow.show();
	this.queryChooseOrderPanel.form.reset();
	this.chooseOrderListPanel.store.removeAll();
}

com.keensen.ump.produce.component.workorder.PackdutyMgr.prototype.onChooseOrder = function() {
	...
	for (var i = 0; i < records.length; i++) {
		...
		var r = new Ext.data.Record({
					...
					productOrder : 1,
					packCode : this.defaultPackCode
				})
		store.add(r);
	}
}

com.keensen.ump.produce.component.workorder.PackdutyMgr.prototype.onDelArrange = function() {
...
com.keensen.ump.produce.component.workorder.PackdutyMgr.prototype.onChooseCode = function() {
	this.chooseMachineWindow.setTitle('选择机台');
	this.chooseMachineWindow.show();
}

com.keensen.ump.produce.component.workorder.PackdutyMgr.prototype.onSelectCode = function() {
	...
	if (!A.getSelectionModel().getSelected()) {
		...
		if (this.window4Arrange.isVisible()) {
			...
			rec2.set('packCode', arr.join(','));
			rec2.commit();
			A.refresh();

		}
		if (this.editWindow.isVisible()) {
			this.editWindow.packCode.setValue(arr.join(','));
		}
...
com.keensen.ump.produce.component.workorder.PackdutyMgr.prototype.onSaveArrange = function() {
	var _this = this;
	...
	Ext.Ajax.request({
		url : "com.keensen.ump.produce.component.workorder2.addBatchPackDuties.biz.ext",
		method : "post",
		jsonData : {
			'duties' : duties
		},
...
com.keensen.ump.produce.component.workorder.PackdutyMgr.prototype.onEdit = function() {
	this.listPanel.onEdit();
}

com.keensen.ump.produce.component.workorder.PackdutyMgr.prototype.onViewList = function() {
...
com.keensen.ump.produce.component.workorder.PackdutyMgr.prototype.onRefreshList = function() {
	var records = this.listPanel.getSelectionModel().getSelections();
	if (records.length != 1) {
		Ext.Msg.alert("系统提示", "请选择一条记录！")
		return false;
	...
	this.requestMask.show();
	Ext.Ajax.request({
		url : "com.keensen.ump.produce.component.workorder2.savePackDutyList.biz.ext",
		method : "post",
		jsonData : {
