	...
	this.initListPanel = function() {
		...
		this.listPanel = this.listPanel || new Ext.fn.EditListPanel({
			...
			tbar : [{
						...
						//超期与预警规则说明:
						//1、自用膜片超期（超过将接受膜片呆滞考核）标准:ULP与BW(含HW)系列为84个自然日,NF、SW系列膜片为174个自然日;
						//2、自用膜片预警（提醒计划员强制使用或启动报废）标准:ULP与BW(含HW)为70个自然日,NF、SW系列标准为160个自然日; 
						//3、发货膜片超期（给予膜片事业部业务员考核后调至AB仓自用）标准:ULP与BW(含HW)系列为30个自然日,NF、SW系列膜片为60个自然日;
