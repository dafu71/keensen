 ...
 * 二维码生成
 * 打印注意：
 * convas方式需要toDataURL("image/png")转化，否则会模糊；table方式打印时需选择打印背景图片
 */
 /*使用示例
$('#qrcodeCanvas').qrcode({
			render : 'convas', // 绘制方式，convas/table，html5浏览器才能支持convas方式
			text : 'ADMINC26CD07634435F324F412EAF058709F4',// 二维码内容
			width : '380', // 二维码宽度
			height : '380', // 二维码高度
			correctLevel : 1,// 纠错级别, L : 1, M : 0, Q : 3, H : 2
			background : '#ffffff', // 二维码的后景色
			foreground : '#000000', // 二维码的前景色
			imgWidth : 180,// logo图片宽度，未指定则默认为二维码宽度的20%
			imgHeight : 180,// log图片高度，未指定则默认为二维码高度的20%
			src : 'lyrun.png' // logo图片地址
		});
*/
...
for (function(a) {
	a.fn.qrcode = function(b) {
		...
		return 'string' == typeof b && (b = {
			text : b
					...
					height : 256,
					// logo图片的高度和宽度
					imgWidth : b.imgWidth || b.width / 5,
					imgHeight : b.imgHeight || b.height / 5,
					typeNumber : -1,
			...
			for (a.addData(utf16to8(b.text)), a.make(), c = document.createElement('canvas'), c.width = b.width, c.height = b.height, d = c
					.getContext('2d'), b.src && (e = new Image(), e.src = b.src, e.onload = function() {
				...
				for (i = 0; i < a.getModuleCount(); i++) {
					d.fillStyle = a.isDark(h, i) ? b.foreground : b.background, j = Math.ceil((i + 1) * f) - Math.floor(i * f), k = Math.ceil((h + 1)
							* f)
							- Math.floor(h * f), d.fillRect(Math.round(i * f), Math.round(h * g), j, k)
				}
			}
			return c
		}, d = function() {
			...
			for (c.addData(utf16to8(b.text)), c.make(), d = a('<table></table>').css('width', b.width + 'px').css('height', b.height + 'px').css(
					'border', '0px').css('border-collapse', 'collapse').css('background-color', b.background), e = b.width / c.getModuleCount(), f = b.height
					/ c.getModuleCount(), g = 0; g < c.getModuleCount(); g++) {
	...
	makeImpl : function(a, b) {
		...
		this.setupPositionProbePattern(0, 0), this.setupPositionProbePattern(this.moduleCount - 7, 0), this.setupPositionProbePattern(0,
				this.moduleCount - 7), this.setupPositionAdjustPattern(), this.setupTimingPattern(), this.setupTypeInfo(a, b), this.typeNumber >= 7
				&& this.setupTypeNumber(a), null == this.dataCache
				&& (this.dataCache = QRCode.createData(this.typeNumber, this.errorCorrectLevel, this.dataList)), this.mapData(this.dataCache, b)
	},
			...
			if (!(-1 >= a + c || this.moduleCount <= a + c)) {
				for (d = -1; 7 >= d; d++) {
					-1 >= b + d
							...
							|| (this.modules[a + c][b + d] = c >= 0 && 6 >= c && (0 == d || 6 == d) || d >= 0 && 6 >= d && (0 == c || 6 == c)
									|| c >= 2 && 4 >= c && d >= 2 && 4 >= d ? !0 : !1)
				}
			}
		}
	...
	createMovieClip : function(a, b, c) {
		...
		for (this.make(), f = 0; f < this.modules.length; f++) {
			for (g = f * e, h = 0; h < this.modules[f].length; h++) {
				i = h * e, j = this.modules[f][h], j
						&& (d.beginFill(0, 100), d.moveTo(i, g), d.lineTo(i + e, g), d.lineTo(i + e, g + e), d.lineTo(i, g + e), d.endFill())
			}
		}
		return d
	},
	...
	setupTypeInfo : function(a, b) {
		...
		for (e = 0; 15 > e; e++) {
			f = !a && 1 == (1 & d >> e), 6 > e ? this.modules[e][8] = f : 8 > e
					? this.modules[e + 1][8] = f
					: this.modules[this.moduleCount - 15 + e][8] = f
		}
		for (e = 0; 15 > e; e++) {
			f = !a && 1 == (1 & d >> e), 8 > e ? this.modules[8][this.moduleCount - e - 1] = f : 9 > e
	...
	mapData : function(a, b) {
		...
		for (g = this.moduleCount - 1; g > 0; g -= 2) {
			for (6 == g && g--;;) {
				for (h = 0; 2 > h; h++) {
					null == this.modules[d][g - h]
							&& (i = !1, f < a.length && (i = 1 == (1 & a[f] >>> e)), j = QRUtil.getMask(b, d, g - h), j && (i = !i), this.modules[d][g
									- h] = i, e--, -1 == e && (f++, e = 7))
				}
	...
	for (h = 0; h < b.length; h++) {
		...
		for (c += i, l = QRUtil.getErrorCorrectPolynomial(j), m = new QRPolynomial(f[h], l.getLength() - 1), n = m.mod(l), g[h] = new Array(l
				.getLength()
				- 1), k = 0; k < g[h].length; k++) {
	...
	getLostPoint : function(a) {
		...
		for (d = 0; b - 1 > d; d++) {
			for (e = 0; b - 1 > e; e++) {
				j = 0, a.isDark(d, e) && j++, a.isDark(d + 1, e) && j++, a.isDark(d, e + 1) && j++, a.isDark(d + 1, e + 1) && j++, (0 == j || 4 == j)
						&& (c += 3)
			}
		}
		for (d = 0; b > d; d++) {
			for (e = 0; b - 6 > e; e++) {
				a.isDark(d, e) && !a.isDark(d, e + 1) && a.isDark(d, e + 2) && a.isDark(d, e + 3) && a.isDark(d, e + 4) && !a.isDark(d, e + 5)
						&& a.isDark(d, e + 6) && (c += 40)
			}
		}
		for (e = 0; b > e; e++) {
			for (d = 0; b - 6 > d; d++) {
				a.isDark(d, e) && !a.isDark(d + 1, e) && a.isDark(d + 2, e) && a.isDark(d + 3, e) && a.isDark(d + 4, e) && !a.isDark(d + 5, e)
						&& a.isDark(d + 6, e) && (c += 40)
			}
		}
		for (k = 0, e = 0; b > e; e++) {
...
QRPolynomial.prototype = {
	...
	mod : function(a) {
...
}, QRRSBlock.RS_BLOCK_TABLE = [[1, 26, 19], [1, 26, 16], [1, 26, 13], [1, 26, 9], [1, 44, 34], [1, 44, 28], [1, 44, 22], [1, 44, 16], [1, 70, 55],
		[1, 70, 44], [2, 35, 17], [2, 35, 13], [1, 100, 80], [2, 50, 32], [2, 50, 24], [4, 25, 9], [1, 134, 108], [2, 67, 43],
		[2, 33, 15, 2, 34, 16], [2, 33, 11, 2, 34, 12], [2, 86, 68], [4, 43, 27], [4, 43, 19], [4, 43, 15], [2, 98, 78], [4, 49, 31],
		[2, 32, 14, 4, 33, 15], [4, 39, 13, 1, 40, 14], [2, 121, 97], [2, 60, 38, 2, 61, 39], [4, 40, 18, 2, 41, 19], [4, 40, 14, 2, 41, 15],
		[2, 146, 116], [3, 58, 36, 2, 59, 37], [4, 36, 16, 4, 37, 17], [4, 36, 12, 4, 37, 13], [2, 86, 68, 2, 87, 69], [4, 69, 43, 1, 70, 44],
		...
		[11, 36, 12, 5, 37, 13], [5, 109, 87, 1, 110, 88], [5, 65, 41, 5, 66, 42], [5, 54, 24, 7, 55, 25], [11, 36, 12], [5, 122, 98, 1, 123, 99],
		[7, 73, 45, 3, 74, 46], [15, 43, 19, 2, 44, 20], [3, 45, 15, 13, 46, 16], [1, 135, 107, 5, 136, 108], [10, 74, 46, 1, 75, 47],
		[1, 50, 22, 15, 51, 23], [2, 42, 14, 17, 43, 15], [5, 150, 120, 1, 151, 121], [9, 69, 43, 4, 70, 44], [17, 50, 22, 1, 51, 23],
