com.zoomlion.hjsrm.settleaccounts.ManageMgr.prototype.initEvent = function() {
	...
	this.queryPanel.mon(this.queryPanel, 'query', function(form, vals) {
		/*var bukrs = this.queryPanel.bukrs.getValue();
		var werks = this.queryPanel.werks.getValue();
		
		if(Ext.isEmpty(bukrs)){
			Ext.Msg.alert("系统提示", "请选择公司!");
			return;
		}
		
		if(Ext.isEmpty(werks)){
			Ext.Msg.alert("系统提示", "请选择工厂!");
			return;
		}*/
		...
		if (Ext.isEmpty(ekorg)) {
            ...
            var jzdate = '2017-06-01';
            if(start < jzdate && end >= jzdate){
            	Ext.Msg.alert("系统提示", "参考凭证过账间隔日期不能跨2017-06-01！");
				return;
            }
			...
			//物料凭证号改为底盘VN码
			var vnbms2 = this.queryPanel.vnbms2.getValue();

			// 换行符替换
			var regEx = new RegExp("\\n", "gi");
			vnbms2 = vnbms2.replace(regEx, ",");
			...
			if (!Ext.isEmpty(vnbms2)) {
				...
				str += "'0'";
				this.queryPanel.vnbms.setValue(str);
				vals["condition/vnbms"] = str;

			}else{
				this.queryPanel.vnbms.setValue('');
				vals["condition/vnbms"] = '';
			}
	...
	if (Ext.isEmpty(ekorg)) {
		...
		//物料凭证号改为底盘VN码
			...
			if (!Ext.isEmpty(vnbms2)) {
				...
				str += "'0'";
				this.queryPanel.vnbms.setValue(str);
				daochu["condition/vnbms"] = str;

			}else{
				this.queryPanel.vnbms.setValue('');
				daochu["condition/vnbms"] = '';
			}
